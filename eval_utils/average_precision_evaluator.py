...
class Evaluator:
    ...
    def get_num_gt_per_class(self,
        ...
        num_gt_per_class = np.zeros(shape=(self.n_classes+1), dtype=np.int64)
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            true_pos = np.zeros(len(predictions), dtype=np.int64) # 1 for every prediction that is a true positive, 0 otherwise
            false_pos = np.zeros(len(predictions), dtype=np.int64) # 1 for every prediction that is a false positive, 0 otherwise
                ...
                if gt_match_overlap < matching_iou_threshold:
                    ...
                    if not (ignore_neutral_boxes and eval_neutral_available) or (eval_neutral[gt_match_index] == False):
                        ...
                        if not (image_id in gt_matched):
                            ...
                            true_pos[i] = 1
                            gt_matched[image_id] = np.zeros(shape=(gt.shape[0]), dtype=np.bool_)
